...
class GPUEngine(mp.Process):
    ...
    def set_ready_notifier(self, ready_event, ready_flag):
        """Allow the parent process to wait for CUDA initialization."""
    ...
    def _signal_ready(self, success):
        if self._ready_notified:
            return
        if self._ready_flag is not None:
            self._ready_flag.value = 1 if success else -1
    ...
    def _execute_mine(self, req):
        try:
            ...
            # Calculate grid size based on configured batch size
            from core.config import config
            batch_size = config.get('gpu.batch_size', 1000000)
            grid_size = max(1, batch_size // block_size)
